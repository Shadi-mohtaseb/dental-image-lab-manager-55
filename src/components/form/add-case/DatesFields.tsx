...
export function DatesFields({ form }: { form: any }) {
  // التأكد من تعبئة تاريخ الاستلام افتراضياً عند فتح النموذج
  useEffect(() => {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {/* حقل تاريخ التسليم (الآن في العمود الأول - يسار) */}
      <FormField
                ...
                <Calendar
                  ...
                  onSelect={(date) => {
                    if (date) {
                      ...
                      field.onChange(""); // يمكن حذف التاريخ
                    }
                  }}
                  initialFocus
      ...
      />
      {/* حقل تاريخ الاستلام (الآن في العمود الثاني - يمين) */}
      <FormField
        control={form.control}
        name="delivery_date"
        render={({ field }) => (
