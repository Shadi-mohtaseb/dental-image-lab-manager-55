...
export function CasesFilterBar({
  ...
  return <div className="p-6 flex flex-col gap-4">
      <div className="flex gap-4 items-center flex-wrap">
        <div>
          <Input placeholder="بحث نصي عن اسم المريض..." value={searchTerm} onChange={e => onSearchTermChange(e.target.value)} className="min-w-[200px] max-w-[270px]" />
        </div>
        ...
        <Select value={selectedDoctorId} onValueChange={onDoctorChange}>
          ...
          <SelectContent>
            ...
            {doctors.map((doctor: any) => <SelectItem key={doctor.id} value={doctor.id}>
                {doctor.name}
              </SelectItem>)}
          </SelectContent>
        </Select>
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <Badge variant={!workTypeFilter ? "default" : "outline"} className="cursor-pointer hover:bg-primary hover:text-white" onClick={() => onWorkTypeFilterChange("")}>
          الكل
        </Badge>
        {workTypes.map(({
        label,
        color
      }) => <Badge key={label} variant={workTypeFilter === label ? "default" : "outline"} className={`cursor-pointer hover:bg-${color} hover:text-white`} onClick={() => onWorkTypeFilterChange(label)}>
            {label}
          </Badge>)}
      </div>
    </div>;
