...
import { Users, UserPlus, Receipt, FileText, TrendingUp, Calendar, DollarSign, Activity, Wallet, Plus, CreditCard } from "lucide-react";
...
const Index = () => {
  ...
  function computeDoctorsDebt() {
    const doctorsCases = doctors.reduce((acc, doc) => {
      const doctorCasesSum = cases.filter(c => c.doctor_id === doc.id).reduce((sum, c) => sum + (Number(c.price) || 0), 0);
  ...
  const {
    ...
    queryFn: async () => {
      let {
        data,
        error
      } = await supabase.from("doctor_transactions").select("*");
      ...
      <Card className="animate-slide-up">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickActions.map((action, index) => <Button key={index} onClick={action.action} className={`${action.color} text-white h-auto p-6 flex flex-col items-center gap-3 hover:scale-105 transition-transform relative overflow-hidden`} style={action.backgroundImage ? {
            backgroundImage: `url('${action.backgroundImage}')`,
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            backgroundRepeat: 'no-repeat'
          } : {}}>
                {action.backgroundImage && <div className="absolute inset-0 bg-black/40 z-0"></div>}
